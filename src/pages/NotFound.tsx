...
const NotFound = () => {
  ...
  useEffect(() => {
    // This will run client-side and set the status in the response object
  ...
  return (
    <>
      <Helmet>
        <title>Page Not Found - LeetCode Buddy</title>
        <meta name="robots" content="noindex" />
        {/* This won't change the actual HTTP status, but can help with analytics */}
        <script>{`
          if (typeof window.__NEXT_DATA__ !== 'undefined') {
            window.__NEXT_DATA__.props.pageProps.statusCode = 404;
          }
        `}</script>
      </Helmet>
